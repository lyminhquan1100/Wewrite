...
import com.arcane.app.repository.ImageRepository;
...
/**
 * REST controller for managing {@link com.arcane.app.domain.Image}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Image> partialUpdateImage(@PathVariable(value = "id", required = false) final Long id, @RequestBody Image image)
        ...
        Optional<Image> result = imageRepository
            ...
            .map(existingImage -> {
                ...
                if (image.getPathImage() != null) {
                    existingImage.setPathImage(image.getPathImage());
                }
