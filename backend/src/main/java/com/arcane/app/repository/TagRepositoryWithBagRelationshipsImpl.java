...
public class TagRepositoryWithBagRelationshipsImpl implements TagRepositoryWithBagRelationships {
    ...
    public Optional<Tag> fetchBagRelationships(Optional<Tag> tag) {
        return tag.map(this::fetchProducts);
    }
    ...
    public List<Tag> fetchBagRelationships(List<Tag> tags) {
        return Optional.of(tags).map(this::fetchProducts).orElse(Collections.emptyList());
    }

    Tag fetchProducts(Tag result) {
        return entityManager
            .createQuery("select tag from Tag tag left join fetch tag.products where tag is :tag", Tag.class)
            .setParameter("tag", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Tag> fetchProducts(List<Tag> tags) {
        return entityManager
            .createQuery("select distinct tag from Tag tag left join fetch tag.products where tag in :tags", Tag.class)
